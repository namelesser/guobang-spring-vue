              ...
              <n-gi v-for="field in fields" :key="field.key">
                <n-form-item :label="field.label">
                  <n-select
                    ...
                    :options="field.collection === '__route_receiver__' ? receiverOptions : (field.collection === '__route_sender__' ? senderOptions : options(field.collection))"
                    clearable
                    filterable
...
const receiverOptions = computed(() => routeReceivers.value.map(value => ({ label: value, value })))
const senderOptions = computed(() => routeSenders.value.map(value => ({ label: value, value })))
...
function options(cat?: string) {
  if (cat === '__route_receiver__') return routeReceivers.value.map(value => ({ label: value, value }))
  if (cat === '__route_sender__') return routeSenders.value.map(value => ({ label: value, value }))
...
async function loadCollections() {
  ...
  try {
    const res = await rateApi.list() as any
    allRates.value = res.rates || res || []
...
function updateRouteReceivers() {
  ...
  routeReceivers.value = dests.sort((a: string, b: string) => a.localeCompare(b, 'zh-CN'))
}
...
function updateRouteSenders() {
  ...
  routeSenders.value = senders.sort((a: string, b: string) => a.localeCompare(b, 'zh-CN'))
}
